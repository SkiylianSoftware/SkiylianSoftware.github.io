...
{% for pl in playlists %}
  <a href="{{ pl.url }}" target="_blank" class="playlist-row btn"
    ...
    <div class="playlist-row-info">
      ...
      {% if pl.description %}
        {% capture nl %}
{% endcapture %}{% capture tnl %}{{ nl }}{{ nl }}{{ nl }}{% endcapture %}{% capture qnl %}{{ nl }}{{ nl }}{{ nl }}{{ nl }}{% endcapture %}
        <p class="playlist-row-desc">{{ pl.description | replace: qnl, nl | replace: tnl, nl | newline_to_br }}</p>
      {% endif %}

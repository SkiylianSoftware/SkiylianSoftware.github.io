    ...
    {% for video in sorted %}
      <div class="video-card" data-video-id="{{ video.video_id }}" data-title="{{ video.title | escape }}"
        ...
        <div class="card-body">
          ...
          {% if video.series %}
            <div class="series-badge">{{ video.series.game }} &middot; Ep {{ video.series.episode_number }}</div>
          {% endif %}
...
function openPlayer(el) {
  ...
  if (next) {
    ...
    nextBox.innerHTML = '<p class="next-label">Watch next</p><div class="next-card" onclick="closePlayer();setTimeout(function(){var c=document.querySelector(\'[data-video-id=\\\'' + nid + '\\\']\');if(c)openPlayer(c);},100)"><span class="next-title">' + ntitle + '</span></div>';
...
.sort-bar label { font-size: 0.85rem; opacity: 0.7; margin-right: 0.25rem; }
.sort-btn {
...
.random-btn { font-size: 1rem; padding: 0.3rem 0.6rem; }

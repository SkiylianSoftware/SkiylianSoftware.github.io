...
def main():
    ...
    games_data = (read_json("games.json") or {}).get("games", {}) or {}
    ...
    for v in all_videos:
        ...
        if gname and pub:
            game_dates.setdefault(gname, []).append(pub)
    ...
    for gname, g in games_data.items():
        ...
        if first and latest:
            try:
                fd = datetime.strptime(first[:10], "%Y-%m-%d")
